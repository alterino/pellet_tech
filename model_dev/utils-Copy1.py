...
def get_extruder_metadata(path, id_str=None):
    ...
    processed_labels_map = {}
    for field_key in field_mapping:
        if field_key.lower() == 'time':
            continue
        processed_labels_map[column_mapping[field_key]] = field_mapping[field_key].strip()

    raw_fieldnames = sorted(processed_labels_map.keys())
    time_keys = [fieldname + '.TIME' for fieldname in raw_fieldnames]

    keys_of_interest = sorted(raw_fieldnames + time_keys)

    return keys_of_interest, time_keys, raw_fieldnames, processed_labels_map
...
def merge_data_with_time(
    df, 
    resample_rate=None, resample_func=None, downsample_func='mean',
    verbose=False, get_series_list=False, variance_sample_rate='1S'
):
    """
    ...
    for column in df.columns:
        if not column.endswith('.TIME'):
            ...
            if time_column in df.columns:
                ...
                if 'PT' in column:
                    local_variance = series.rolling(variance_sample_rate).var()
                    local_variance = local_variance.loc[series.index]
                ...
                if downsample_func == 'max':
                    series = groupby(level=0).max()
                elif downsample_func == 'median':
                    series = groupby(level=0).median()
                else:
    ...
    merged_df['sample_gap_seconds'] = merged_df.index.to_series().diff().apply(lambda x: x.total_seconds())
